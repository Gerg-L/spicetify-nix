...
let
  ...
  needExperimental =
    lib.lists.any
      (
        item:
        (
          if (builtins.hasAttr "experimentalFeatures" item) then
  ...
  overridenXpui1 =
    builtins.mapAttrs (name: value: (lib.trivial.mergeAttrs cfgXpui.${name} value))
  ...
  overridenXpui2 =
    builtins.mapAttrs
      ...
      (mkXpuiOverrides cfg);
in
builtins.toFile "config-xpui.ini" (spiceLib.createXpuiINI overridenXpui2)
